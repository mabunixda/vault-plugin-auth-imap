...
func (b *backend) pathAuthRenew(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(fmt.Sprintf("failed to validate role %s during renewal: {{err}}", roleName), err)
	}
...
const (
	backendHelp = `
The imap backend plugin allows authentication using IMAP Servers.
`
)
